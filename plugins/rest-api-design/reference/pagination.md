...
## MUST support pagination {#rule-159}
...
- **Cursor-based pagination** — aka key-based pagination: a unique key
  ...
  [Twitter API](https://dev.twitter.com/overview/api/cursoring) or
  [Facebook API](https://developers.facebook.com/docs/graph-api/results))
...
## SHOULD prefer cursor-based pagination, avoid offset-based pagination {#rule-160}
...
the *cursor-based pagination* section below).
...
- `prev`: the link or cursor pointing to the previous page.
It is not provided, if it is the first page.
- `next`: the link or cursor pointing to the next page.
