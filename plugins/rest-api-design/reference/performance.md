...
- `ETag` and `If-Match`/`If-None-Match` headers to avoid re-fetching of
  unchanged resources (see [#182](http-headers.md#rule-182))
- `Prefer` header with `return=minimal` or `respond-async` to anticipate reduced
  processing requirements of clients (see [#181](http-headers.md#rule-181))
...
See [#137](urls.md#rule-137) for naming, e.g. "embed" for steering of embedded
...
using SSL and OAuth authorization ([#104](security.md#rule-104)), resource update and invalidation
...
- In general, you should support `ETag` Together With `If-Match`/`If-None-Match` Header ([#182](http-headers.md#rule-182)) on all *cacheable* endpoints.
...
failed `HEAD` or `GET` request with `If-None-Match: <entity-tag>` ([#182](http-headers.md#rule-182))
instead of `412`. For `ETag` see also [#182](http-headers.md#rule-182).

#### Default Header Values

The default setting for `Cache-Control` should contain the `private` directive
for endpoints with standard OAuth authorization ([#104](security.md#rule-104)), as well as the
