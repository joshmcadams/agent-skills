...
## MUST use JSON as payload data interchange format {#rule-167}

Use JSON ([RFC 7159](https://tools.ietf.org/html/rfc7159)) to represent structured (resource) data passed
...
use an array -- see also [#110](compatibility.md#rule-110).
...
- consist of [valid Unicode strings](https://tools.ietf.org/html/rfc7493#section-2.1), i.e. must not
  contain non-characters or surrogates, and
- contain only [unique member names](https://tools.ietf.org/html/rfc7493#section-2.3) (no duplicate
  names).


## SHOULD design single resource schema for reading and writing {#rule-252}
...
ignored or rejected by a server. Both are in line with the idea behind the compatibility guidance ([#109](compatibility.md#rule-109)),
...
- Postgres cannot handle `\u0000` in `jsonb` and `text` types (see
  [datatype-json](https://www.postgresql.org/docs/current/datatype-json.html)).
...
## MAY pass non-JSON media types using data specific standard formats {#rule-168}
...
may be provided, but only additionally to JSON as default format using content negotiation ([#244](data-formats.md#rule-244)),
for specific use cases where clients may not interpret the payload structure.


## SHOULD use standard media types {#rule-172}
...
`application/json` (or `application/problem+json` for [#176](http-status-codes-and-errors.md#rule-176)).
...
> adaptation versions APIs through the URL path (see [#114](compatibility.md#rule-114)) and does **not** use
> media type versioning (see [#115](compatibility.md#rule-115)), that exception no longer applies: keep the
> standard `application/json` media type and put the major version in the path
> (`/api/v1/...`) instead.


## SHOULD pluralize array names {#rule-120}
...
<a id="rule-125"></a>
## SHOULD declare enum values using UPPER_SNAKE_CASE string {#rule-240}
...
declaring possible values for a rule 137 ([#137](urls.md#rule-137)) [`sort` parameter].
...
**Hint:** Use `format: date-time` or `format: date` as required in [#238](data-formats.md#rule-238).
...
## MUST use same semantics for `null` and absent properties {#rule-123}
...
|---|---|---|---|
| `true`  | `true`  | ❌ No  | ✔️ Yes |
| `false` | `true`  | ✔️ Yes | ✔️ Yes |
| `true`  | `false` | ❌ No  | ❌ No |
| `false` | `false` | ✔️ Yes | ❌ No |
...
dedicated [Null object](https://en.wikipedia.org/wiki/Null_object_pattern), e.g.
`{}` compared to `{"id":"42"}`.
...
[JSON-B](https://developer.ibm.com/articles/j-javaee8-json-binding-3/)). Especially
...
The only exception to this rule is JSON Merge Patch [RFC 7396](https://tools.ietf.org/html/rfc7396)) which
uses `null` to explicitly indicate property deletion while absent properties are
ignored, i.e. not modified.


## MUST not use `null` for boolean properties {#rule-122}
...
## SHOULD not use `null` for empty arrays {#rule-124}

Empty array values can unambiguously be represented as the empty list, `[]`.


## MUST use common field names and semantics {#rule-174}
...
- `id`: the identity of the object. If used, IDs must be opaque strings
...
- `xyz_id`: an attribute within one object holding the identifier of
  ...
  **Exception**: We use `customer_number` instead of `customer_id` for customer facing
  ...
  Naming Decision: `customer_number` vs `customer_id`).
- `etag`: the `etag` of an embedded sub-resource ([#158](performance.md#rule-158)). It
  typically is used to carry the `ETag` for subsequent `PUT`/`PATCH` calls
  (see `ETag` ([#182](http-headers.md#rule-182)) and the *ETags in result entities* section).

Further common fields are defined in [#235](#rule-235).
The following guidelines define standard objects and fields:

- [#248](pagination.md#rule-248)
- [#249](#rule-249)
- [#173](#rule-173)
...
## MUST use the common address fields {#rule-249}
...
## MUST use the common money object {#rule-173}
...
See [Stack Overflow](http://stackoverflow.com/a/3730040/342852) for more
info.
