...
- the *JSON Guidelines* section
- the *Hypermedia* section


## MUST treat events as part of the service interface {#rule-194}
...
with the "API first" principle in mind as described in the *Introduction* section.


## MUST make event schema available for review {#rule-195}
...
## MUST indicate ownership of event types {#rule-207}
...
## MUST carefully define the compatibility mode {#rule-245}
...
principle described in the guideline's *API Design Principles* section.
...
## MUST ensure event schema conforms to OpenAPI schema object {#rule-196}
...
The [OpenAPI Schema Object](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/2.0.md#schemaObject)
is an **extended subset** of
[JSON Schema Draft 4](http://json-schema.org/). For convenience,
we highlight some important differences below. Please refer to the
[OpenAPI Schema Object specification](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/2.0.md#schemaObject)
for details.
...
[vendor extensions](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/2.0.md#vendorExtensions) can be used in event type schema, but it might be the case
...
## SHOULD avoid `additionalProperties` in event type schemas {#rule-210}
...
## MUST ensure that events conform to an event category {#rule-198}
...
metadata field being reserved for standard
information (the contents of metadata are
described further down in this section).

**Note:**

- The General Event was called a *Business Event* in earlier versions of
...
## MUST provide unique event identifiers {#rule-211}
...
## MUST use general events to signal steps in business processes {#rule-201}
...
## SHOULD provide explicit event ordering for general events {#rule-203}
...
## MUST use data change events to signal mutations {#rule-202}
...
## MUST provide explicit event ordering for data change events {#rule-242}
...
## SHOULD use the hash partition strategy for data change events {#rule-204}
...
## MUST be robust against duplicates when consuming events {#rule-214}
...
## MUST ensure that events define useful business resources {#rule-199}
...
## SHOULD ensure that data change events match the APIs resources {#rule-205}
