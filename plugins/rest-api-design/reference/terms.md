...
- **OpenAPI Specification** (OAS) -- the REST API specification format defined as standard by the [OpenAPI Initiative](https://www.openapis.org/). We currently encourage using [OpenAPI 3.1 version](https://spec.openapis.org/oas/v3.1.0.html) for API specifications.
- **API Specification** (aka [OpenAPI Description (OAD)](https://spec.openapis.org/oas/latest.html#openapi-description-structure) -- the document in OAS format specifying an API, its syntax and semantics. It should provide all information for clients to use the API, and serves as the interface definition of the API implementation.
